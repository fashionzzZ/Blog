...
**链表的特点：**
在内存中可以存在任何地方，不要求连续。
每一个数据都保存了下一个节点的内存地址，通过这个地址找到下一个数据。
增加数据和删除数据很容易。
查找数据时效率低，因为不具有随机访问性，所以访问某个位置的数据都要从第一个数据开始遍历。
...
扩容会分配一个新数组，将原数组中的元素拷贝到新数组中，并释放元数组的内存。
...
**拦截器：**访问web服务器的时候，在你调用的方法前做一个拦截，加上我想要的任意操作。比喻记录操作日志，加上一些业务(AOP)。
